...
# Create desktop launchers (KDE6/Wayland compatible)
...
# UC-1 Control Panel launcher (Wayland compatible)
...
[General]
Command=/bin/bash -c 'export HSA_OVERRIDE_GFX_VERSION=11.0.0; source /home/ucadmin/ai-env/bin/activate && echo "AI Environment Active (ROCm 6.3.2)" && exec zsh'
Name=AI Environment
Parent=FALLBACK/
EOF

# AI Terminal launcher (Wayland compatible)
...
# Jupyter Lab launcher (Wayland compatible)
...
# UC-Monitor launcher (Wayland compatible)
...
if [ -d "\$UC1_PATH" ] && [ -f "\$UC1_PATH/docker-compose.yaml" ]; then
    ...
    if command -v uc-monitor >/dev/null 2>&1; then
        /usr/local/bin/uc-monitor
    else
    ...
    if command -v xrt-smi >/dev/null 2>&1; then
        xrt-smi examine 2>/dev/null || echo "NPU not detected"
    else
        echo "xrt-smi not installed; NPU status unavailable"
    fi
else
...
EOF
    ...
    echo -e "${YELLOW}⚠️ .zshrc not found, skipping aliases${NC}"
fi
...
echo -e "  - Run '04-theme-customization.sh' for custom branding"
echo -e "  - Start services (if not running): uc-launch"
